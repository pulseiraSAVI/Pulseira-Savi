...
function viewResultadoNivel1(root) {
  ...
  var esc = domUtil.escapeHtml;

  function nomeCabecalho(nomeCompleto) {
    var partes = (nomeCompleto || "").trim().split(/\s+/);
    if (partes.length <= 2) return esc(nomeCompleto);
    return esc(partes[0] + " " + partes[partes.length - 1]);
  }
  ...
  function conteudoUtente() {
    ...
    return (
      ...
      '<p class="card-value">' + esc(paciente.nome) + "</p>" +
      '<p class="card-sub">Nº de utente: ' + esc(paciente.numero_utente || "—") + "</p>" +
      '<p class="card-sub">Morada: ' + esc(paciente.morada || "—") + "</p>" +
      "</div>" +

      '<div class="section-title">Contacto de familiares</div>' +
      '<div class="contact-card">' +
      '<div><div class="who">Familiares</div><div class="name">' + esc(paciente.contacto_familia || "Sem contacto registado") + "</div></div>" +
      ...
      '<div class="contact-card">' +
      '<div><div class="who">Emergência</div><div class="name">' + esc(paciente.contacto_emergencia || "Sem contacto registado") + "</div></div>" +
      '<button class="call-btn" id="btn-ligar-emergencia">' + SAVI_ICONS.telefone() + " Ligar</button>" +
      "</div>" +

      '<div class="section-title">Gestor do caso</div>' +
      '<div class="card">' +
      '<p class="card-value">' + esc(gestor.nome || "Sem gestor do caso registado") + "</p>" +
      (gestor.especialidade ? '<p class="card-sub">' + esc(gestor.especialidade) + "</p>" : "") +
      (gestor.credencial_ordem ? '<p class="card-sub">Nº de Ordem: ' + esc(gestor.credencial_ordem) + "</p>" : "") +
      (gestor.instituicao_servico ? '<p class="card-sub">' + esc(gestor.instituicao_servico) + "</p>" : "") +
      (gestor.telefone_institucional ? '<p class="card-sub">Tel.: ' + esc(gestor.telefone_institucional) + "</p>" : "") +
      "</div>"
    );
  }
  ...
  function conteudoAlgoritmo() {
    return (
      ...
      '<p class="card-value">' + esc(dados.alergias || "Sem registo") + "</p>" +
      "</div>" +

      '<div class="card">' +
      '<p class="card-label">Biometria</p>' +
      '<p class="card-value">' + (dados.peso_kg ? esc(dados.peso_kg) + " kg" : "peso não registado") + " · " + (dados.altura_cm ? esc(dados.altura_cm) + " cm" : "altura não registada") + "</p>" +
      ...
      '<p class="card-value">' + esc(dados.condicao_critica || "Sem registo") + "</p>" +
      (dados.esquema_atuacao_crise ? '<p class="card-sub"><b>Esquema de atuação perante crise:</b> ' + esc(dados.esquema_atuacao_crise) + "</p>" : "") +
      "</div>" +

      '<div class="card">' +
      '<p class="card-label">Esquema de dose</p>' +
      '<p class="card-value">' + esc(dados.esquema_dose || "Sem registo") + "</p>" +
      "</div>" +

      '<div class="card">' +
      '<p class="card-label">Medicação contraindicada</p>' +
      '<p class="card-value">' + esc(dados.medicacao_contraindicada || "Sem registo") + "</p>" +
      "</div>" +
      ...
      '<p class="card-value">' + esc(dados.notas || "Sem notas adicionais.") + "</p>" +
      "</div>" +
  ...
  function conteudoMedicacao() {
    var listaDocs = documentos.length
      ? documentos.map(function (d) {
          return '<div class="doc-item"><span class="doc-icon">' + iconeDocumento(d.tipo) + '</span><div class="doc-info"><div class="doc-nome">' + esc(d.nome_ficheiro) + '</div><div class="doc-meta">' + rotuloTipoDocumento(d.tipo) + " · " + fmtDataCurta(d.enviado_em) + "</div></div></div>";
        }).join("")
    ...
    return (
      ...
      '<p class="card-value">' + esc(dados.medicacao_cronica || "Sem registo") + "</p>" +
      "</div>" +
  ...
  function conteudoConfiguracao() {
    return (
      ...
      '<p class="card-value">' + esc(sessao.nome) + "</p>" +
      '<p class="card-sub">Nº de Ordem: ' + esc(sessao.credencial) + "</p>" +
      '<p class="card-sub">Serviço: ' + esc(sessao.servico || "—") + "</p>" +
      "</div>" +
      '<div class="card">' +
